...
SimpleDownloadManagerDelegate::SimpleDownloadManagerDelegate()
...
void SimpleDownloadManagerDelegate::SetDownloadManager(
    DownloadManager* download_manager) {
  download_manager_ = download_manager;
}

void SimpleDownloadManagerDelegate::Shutdown() {
  Release();
}

bool SimpleDownloadManagerDelegate::DetermineDownloadTarget(
  ...
  BrowserThread::PostTask(
      ...
      FROM_HERE,
      base::Bind(
          &SimpleDownloadManagerDelegate::GenerateFilename,
...
void SimpleDownloadManagerDelegate::GenerateFilename(
    int32 download_id,
    const DownloadTargetCallback& callback,
  ...
  BrowserThread::PostTask(
      ...
      FROM_HERE,
      base::Bind(
          &SimpleDownloadManagerDelegate::OnDownloadPathGenerated,
          this, download_id, callback, suggested_path));
}

void SimpleDownloadManagerDelegate::OnDownloadPathGenerated(
    int32 download_id,
    const DownloadTargetCallback& callback,
...
void SimpleDownloadManagerDelegate::ChooseDownloadPath(
    int32 download_id,
    const DownloadTargetCallback& callback,
    const base::FilePath& suggested_path) {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
...
void SimpleDownloadManagerDelegate::SetDownloadBehaviorForTesting(
